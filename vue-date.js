...
(function(window) {
    function getCalendar(y, m) {
        ...
        for(i = 0; i < lastFix; i++) {
            ...
            r1[i] = {month: lastMonth, day: t, data: lastDate + 1}
        }
        for(i = 0; i < maxDate; i++) {
            t = i + 1;
        ...
        tempalte: `
            <tr>
                <td
                    v-for="item in items"
                    :class = "{
                        'dp-last': month != item.minth,
                        'dp-today': cur == item.data,
                        'dp-select': sel == item.data
                    }"
                >
    ...
    var calendar = Vue.extend({
        ...
        data: function() {
            ...
            var cur = "" + curTime.getFullYear() + "-" + (curTime.getMonth() + 1) + curTime.getTime(); // 当前日期
            var y =　d.getFullYear();
            var m = d.getMonth() + 1;
            var data = getCalendar(d.getFullYear(), d.getMonth() + 1); //显示日历
            return {
                cur: cur,
                sel: sel,
                y: y,
        ...
        template: `
            <div class="input-wrap">
                <input type="text" class="input middle-input" #foucs="foc" v-model="sel">
            </div>
        ...
        methods: {
            ...
            cy: function(flag) {
                ...
                if(flag == -1) {
                    ...
                    this.$emit("init", parseInt(this.y) + 1, thism.m);
                }
            },
            clickNow: function() {
                var t = new Date();
        ...
        events: {
            ...
            click: function(data) {
                ...
                var m = ar[1];
                var y = ar[0];
                this.date = new Date(ar[0], ar[1] - 1, ar[2]).getTime() // 更新时间戳
                if( m !== this.m) {
                    this.y = y;
                    this.m = m;
                    this.data = getCalendar(y, m);
